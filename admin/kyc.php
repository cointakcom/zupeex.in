...
/**
 * ======================================================
 * ADMIN KYC.PHP - KYC Management UI (CSRF FIXED)
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .kyc-card{background:#FFFFFF;border-radius:14px;padding:20px;border:1px solid rgba(125,2,171,0.1);box-shadow:0 4px 15px rgba(125,2,171,0.15)}
        ...
        .kyc-card .user-name{font-size:18px;font-weight:800;color:#000000}
        .kyc-card .user-detail{font-size:13px;color:#555555}
        ...
        .status-badge.verified{background:rgba(16,185,129,0.15);color:#047857}
        ...
        .kyc-image-row{display:flex;gap:10px;margin:12px 0;flex-wrap:wrap}
        .kyc-doc-thumb{width:140px;height:90px;object-fit:cover;border-radius:8px;border:1px solid rgba(125,2,171,0.2);cursor:zoom-in}
        .kyc-card .detail-item{background:#F5E6FF;padding:8px 12px;border-radius:8px}
        .kyc-card .detail-item .label{font-size:11px;color:#555555;text-transform:uppercase;font-weight:700}
        .kyc-card .detail-item .value{font-size:14px;font-weight:800;color:#000000}
        ...
        .btn-action.verify{background:rgba(16,185,129,0.15);color:#047857}
        ...
        .form-group textarea:focus{outline:none;border-color:#7D02AB}
...
<body>
    <div class="admin-container">
        ...
        <div class="stats-bar" id="statsBar">
            ...
            <div class="stat-card"><div class="stat-number" style="color:#FFFFFF" id="statVerified">...</div><div class="stat-label">Verified</div></div>
            <div class="stat-card"><div class="stat-number" style="color:#FFFFFF" id="statRejected">...</div><div class="stat-label">Rejected</div></div>
            <div class="stat-card"><div class="stat-number" style="color:#FFFFFF" id="statTotal">...</div><div class="stat-label">Total</div></div>
        </div>
        ...
        <div class="kyc-list" id="kycList"><div style="text-align:center;padding:40px;color:#000000">Loading...</div></div>
    ...
    <script>
        ...
        function loadStats(){
            fetch(BASE_PATH + '/api/admin_kyc.php?action=get_stats', { credentials: 'include' }).then(handleApiResponse).then(d=>{
        ...
        function loadKycList(){
            document.getElementById('kycList').innerHTML='<div style="text-align:center;padding:40px;color:#000000">Loading...</div>';
            const offset=state.currentPage*state.limit;
            const status=state.status==='all'?'':state.status;
            fetch(`${BASE_PATH}/api/admin_kyc.php?action=list&status=${status}&offset=${offset}&limit=${state.limit}&search=${encodeURIComponent(state.search)}`, { credentials: 'include' }).then(handleApiResponse).then(d=>{
                if(d.success){
                    state.total=d.data.total||0;
                    document.getElementById('kycList').innerHTML=(d.data.documents||[]).map(doc=>{
                        const isBank = doc.document_type === 'bank';
                        let extraFields = '';
                        if (isBank) {
                            extraFields = `<div class="detail-item"><div class="label">Account Holder</div><div class="value">${escapeHtml(doc.bank_account_name||'—')}</div></div><div class="detail-item"><div class="label">Bank Account No.</div><div class="value">${escapeHtml(doc.bank_account_number||'—')}</div></div><div class="detail-item"><div class="label">IFSC</div><div class="value">${escapeHtml(doc.bank_ifsc||'—')}</div></div><div class="detail-item"><div class="label">UPI ID</div><div class="value">${escapeHtml(doc.upi_id||'—')}</div></div>`;
                        } else {
                            extraFields = `<div class="detail-item"><div class="label">Declared Name</div><div class="value">${escapeHtml(doc.kyc_full_name||'⚠️ Not provided')}</div></div><div class="detail-item"><div class="label">DOB</div><div class="value">${escapeHtml(doc.kyc_dob||'—')}</div></div>`;
                        }
                        const imgHtml = (!isBank && doc.document_image_front && doc.document_image_front !== 'n/a')
                            ? `<div class="kyc-image-row"><a href="${BASE_PATH}/api/kyc_image.php?doc_id=${doc.id}&side=front" target="_blank"><img src="${BASE_PATH}/api/kyc_image.php?doc_id=${doc.id}&side=front" class="kyc-doc-thumb" alt="Front"></a>${doc.document_image_back ? `<a href="${BASE_PATH}/api/kyc_image.php?doc_id=${doc.id}&side=back" target="_blank"><img src="${BASE_PATH}/api/kyc_image.php?doc_id=${doc.id}&side=back" class="kyc-doc-thumb" alt="Back"></a>` : ''}</div>`
                            : '';
                        return `<div class="kyc-card">
                        <div class="kyc-header"><div><span class="user-name">${escapeHtml(doc.username||'Unknown')}</span><br><span class="user-detail">📱 ${escapeHtml(doc.mobile||'N/A')} • User #${doc.user_id}</span></div><span class="status-badge ${doc.status}">${escapeHtml(doc.status.toUpperCase())}</span></div>
                        ${imgHtml}
                        <div class="kyc-details"><div class="detail-item"><div class="label">Document Type</div><div class="value">${escapeHtml(String(doc.document_type||'').toUpperCase())}</div></div><div class="detail-item"><div class="label">Document Number</div><div class="value">${escapeHtml(doc.document_number)}</div></div>${extraFields}${doc.rejection_reason?`<div class="detail-item"><div class="label">Rejection Reason</div><div class="value" style="color:#B91C1C">${escapeHtml(doc.rejection_reason)}</div></div>`:''}</div>
                        <div class="action-buttons">${doc.status==='pending'?`<button class="btn-action verify" onclick="verifyKyc(${doc.id})">✅ Verify</button><button class="btn-action reject" onclick="openRejectModal(${doc.id})">❌ Reject</button>`:`<button class="btn-action verify" onclick="verifyKyc(${doc.id})">🔄 Re-verify</button>`}</div></div>`;
                    }).join('')||'<div style="text-align:center;padding:40px;color:#000000">No documents found</div>';
                    renderPagination();
                }
            }).catch(()=>{document.getElementById('kycList').innerHTML='<div style="text-align:center;padding:40px;color:#DC2626">Error loading</div>'});
        }
        ...
        function changePage(delta){const totalPages=Math.max(1,Math.ceil(state.total/state.limit));state.currentPage=Math.min(Math.max(0,state.currentPage+delta),totalPages-1);loadKycList()}
        
        async function verifyKyc(id){
            if(!confirm('Verify this KYC document?'))return;
            const d = await adminPost(BASE_PATH + '/api/admin_kyc.php?action=verify', { id: id });
            if(d.success){showToast('KYC verified!','success');loadStats();loadKycList()}
        ...
        async function confirmReject(){
            ...
            const d = await adminPost(BASE_PATH + '/api/admin_kyc.php?action=reject', { id: parseInt(id), reason: reason });
            if(d.success){showToast('KYC rejected','success');closeModal('rejectModal');loadStats();loadKycList()}
            else showToast(d.message||'Failed','error');
        }
